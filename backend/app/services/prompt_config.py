...
_DEFAULT_PROMPTS: Dict[str, PromptConfig] = {
    "feature-list": PromptConfig(
        ...
        request_description="업로드된 자료에서 주요 기능과 계층 구조, 상세 설명을 CSV로 정리합니다.",
        system_prompt="당신은 소프트웨어 기획 QA 리드입니다. 업로드된 요구사항을 기반으로 기능 정의서를 작성합니다.",
        user_prompt=(
            "요구사항 자료에서 주요 기능을 발췌하여 CSV로 정리하세요. "
            "다음 열을 포함해야 합니다: 개요, 대분류, 중분류, 소분류, 상세 내용. "
            "각 열은 템플릿의 계층 구조에 맞춰 기능 목적과 세부 설명을 요약해야 합니다."
        ),
        user_prompt_sections=[
            PromptSection(
                ...
                content=(
                    ...
                    "2. 기능을 개요-대분류-중분류-소분류 구조로 재구성하며\n"
                    "3. 상세 내용에는 사용 목적, 주요 절차, 참조 근거를 2~3문장으로 요약하세요."
                ),
            ),
            PromptSection(
                id="feature-quality",
                label="품질 기준",
                content=(
                    "- 개요에는 해당 대분류 전체를 한 문장으로 설명하고, 상세 내용에는 실행 조건과 기대 효과를 구체적으로 작성합니다.\n"
            ...
            PromptSection(
                ...
                label="후속 지시",
                content=(
                    "CSV 열 순서는 ‘개요, 대분류, 중분류, 소분류, 상세 내용’으로 고정하고, "
                    "상세 내용은 줄바꿈 없이 자연스러운 문장으로 작성하세요."
                ),
            ),
        ],
